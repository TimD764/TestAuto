...
public class EhuTests
{
    ...
    public void VerifyAboutPage_Test1()
    {
        ...
        // Wait for the 'About' link to be clickable, then click it
    ...
    [Test]
    public void VerifySearch_Test2()
    {
        _driver.Navigate().GoToUrl("https://en.ehuniversity.lt/?s=study+programs");

        _wait.Until(d => d.Url.Contains("?s=study+programs"));

        Assert.That(_driver.Url, Does.Contain("?s=study+programs"),
            "URL did not contain queri");

        var resultLinks = _driver.FindElements(By.CssSelector("a[href*='/studies/']"));
        Assert.That(resultLinks.Count, Is.GreaterThan(0),
            "No links to study programs.");
    }
    ...
    public void VerifyLanguageChange_Test3()
    {
        ...
        Assert.That(bodyText, Does.Match("Apie|Kontaktai|Pradžia"), "No lithuanian text");
    }
